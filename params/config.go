...
var (
	...
	// AllYanzalProtocolChanges contains every protocol change (EIPs) introduced
	// and accepted by the Ethereum core developers into the Yanzal consensus.
	//
	// This configuration is intentionally not using keyed fields to force anyone
	// adding flags to the config to also have to set these fields.
	AllYanzalProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, nil, nil, &YanzalConfig{Length: 30}}
...
type ChainConfig struct {
	...
	Yanzal *YanzalConfig `json:"yanzal,omitempty"`
}
...
// YanzalConfig is the consensus engine configs for proof-of-stake based sealing.
type YanzalConfig struct {
	Length uint64 `json:"length"` //Number of seconds between deciding a winning validator
}

func (c *YanzalConfig) String() string {
	return "yanzal"
}
